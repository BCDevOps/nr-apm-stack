...
describe('ApacheParser', () => {
  ...
  myContainer.bind<Parser>(TYPES.Parser).to(ApacheParser);
  // myContainer.bind<Logger>(TYPES.Logger).to(LoggerImpl);
  myContainer.bind<LoggerService>(TYPES.LoggerService).to(LoggerVoidService);
  test('parse suspicious entry - 001', () => {
  ...
  test('parse suspicious entry - 002', () => {
    const parser = myContainer.get<Parser>(TYPES.Parser);
    // eslint-disable-next-line max-len
