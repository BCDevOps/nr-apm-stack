...
describe('RemoveMetadataParser', () => {
  it('matches documents with metadata', () => {
    ...
    expect(rmMetaData.matches({data: {'@metadata': {}}} as unknown as OsDocument)).toBe(true);
    expect(rmMetaData.matches({data: {'bob': {}}} as unknown as OsDocument)).toBe(false);
  });

  it('apply removes metadata from documents', () => {
    const rmMetaData = new RemoveMetadataParser();
    const test = {data: {'bob': '', '@metadata': {}}} as unknown as OsDocument;
