...
describe('HttpStatusEventOutcomeParser', () => {
  it('matches only if http.response.status_code is set', () => {
    ...
    expect(parser.matches({data: {
      'http': {response: {status_code: 200}}}} as unknown as OsDocument)).toBe(false);
    expect(parser.matches({data: {
      '@metadata': {httpStatusOutcome: true},
      'http': {response: {status_code: 200}}}} as unknown as OsDocument)).toBe(true);
  ...
  it('does not match if event.outcome set already', () => {
    ...
    expect(parser.matches({data: {
      '@metadata': {httpStatusOutcome: true},
      'event': {outcome: 'success'}}} as unknown as OsDocument)).toBe(false);
  });
