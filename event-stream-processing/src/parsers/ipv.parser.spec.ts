...
describe('IpvParser', () => {
  ...
  it('rewrites ipv4 in ipv6', () => {
    ...
    const document = {
      data: {
        source: { ip: '::ffff:10.97.6.1' },
        '@metadata': { ipv: 'source.ip' },
      },
    } as unknown as OsDocument;
  ...
  it('does not rewrite non-ipv4 as ipv6', () => {
    ...
    const document = {
      data: {
        source: { ip: '2001::ffff:ffff:ffff:ffff:ffff:ffff' },
        '@metadata': { ipv: 'source.ip' },
      },
    } as unknown as OsDocument;
    parser.apply(document);
    expect(document.data.source).toEqual({
      ip: '2001::ffff:ffff:ffff:ffff:ffff:ffff',
    });
  });
});
