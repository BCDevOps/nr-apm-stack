...
export class ThreatPhpParser implements Parser {
  ...
  matches(document: OsDocument): boolean {
    return !!(
      ...
      document.data['@metadata'].threatPhp === true
    );
  }
  ...
  apply(document: OsDocument): void {
    const urlOriginal: string = (
      lodash.get(document.data, 'url.original', '') as string
    ).toLowerCase();
    if (
      urlOriginal.indexOf('<php') >= 0 ||
      urlOriginal.indexOf('%3Cphp') >= 0
    ) {
      lodash.set(
        document.data,
        'event.risk_score',
        Math.max(lodash.get(document.data, 'event.risk_score', 0), 1),
      );
    }
    if (
      urlOriginal.indexOf('/wp-content/') >= 0 ||
    ...
    if (
      urlOriginal.indexOf('xdebug_session_start') >= 0 ||
      urlOriginal.indexOf('phpstorm') >= 0
    ) {
      lodash.set(
        document.data,
        'event.risk_score',
    ...
    if (
      urlOriginal.indexOf('token=') >= 0 ||
      urlOriginal.indexOf('password=') >= 0
    ) {
      lodash.set(
        document.data,
        'event.risk_score',
        Math.max(lodash.get(document.data, 'event.risk_score', 0), 10),
      );
    }
    ...
    if (
      urlOriginal.indexOf('win.ini') >= 0 ||
      urlOriginal.indexOf('system32') >= 0
    ) {
      lodash.set(
        document.data,
        'event.risk_score',
