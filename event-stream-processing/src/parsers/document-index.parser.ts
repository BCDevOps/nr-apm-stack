...
export class DocumentIndexParser implements Parser {
  ...
  apply(document: OsDocument): void {
    ...
    const indexPreprodQualifier: string = lodash.get(
      document.data,
      '@metadata.indexPreprodQualifier',
    );
    if (!indexName) {
  ...
  private applyTimestampSubstitution(
    document: OsDocument,
    index: string,
  ): string {
  ...
  private applyDataFieldSubstitution(
    document: OsDocument,
    index: string,
  ): string {
    return index.replace(/<!=[^=]+=!>/gm, (match: string) => {
  ...
  private applyPreprodQualifier(document: OsDocument, index: string): string {
    if (lodash.get(document.data, 'service.environment') === 'production') {
      ...
      return offset > 0
        ? `${index.substring(0, offset)}-preprod${index.substring(offset)}`
        : index;
    }
  }
}
