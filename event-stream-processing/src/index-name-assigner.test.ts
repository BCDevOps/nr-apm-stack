...
test('access log with @timestamp', async () => {
...
test('access log without @timestamp and with event.created', async () => {
    const namer = new IndexNameAssigner()
    const record = lodash.merge({}, APACHE_ACCESS_LOG_EVENT_SIGNATURE, {event:{created:"2021-05-02T18:47:40.314-07:00"}})
    namer.apply(record)
    expect(record).toHaveProperty('_index', 'nrm-logs-access-2021.05.02')
});

test('access log without any timestamp field', async () => {
    const namer = new IndexNameAssigner()
    const record = lodash.merge({}, APACHE_ACCESS_LOG_EVENT_SIGNATURE)
    delete record.event.ingested
    expect(() => {namer.apply(record)}).toThrow('Timestamp field value has not been defined!')
});

test('metric with @timestamp', async () => {
    const namer = new IndexNameAssigner()
...
test('metric without @timestamp and with event.created', async () => {
    const namer = new IndexNameAssigner()
...
test('custom index', async () => {
    const namer = new IndexNameAssigner()
    const record = {event: {kind: 'metric', created: '2021-05-02T18:47:40.314-07:00'}, '@metadata':{index:'blah-%{+YYYY.MM.DD}'}}
    namer.apply(record)
...
test('unexpected event', async () => {
    ...
    expect(() => {namer.apply(record)}).toThrow('Could not map event to an index')
});
