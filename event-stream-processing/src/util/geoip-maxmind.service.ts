...
export class GeoIpMaxmindService implements GeoIpService {
  constructor(
    @inject(TYPES.MaxmindCityLookupService)
    private cityLookup: MaxmindCityLookupService,
    @inject(TYPES.MaxmindAsnLookupService)
    private asnLookup: MaxmindAsnLookupService,
  ) {}
  ...
  public lookup(ipAddress: string): GeoIpServiceResult {
    ...
    if (cityLookupResponse) {
      ...
      if (cityLookupResponse.location) {
        Object.assign(resultGeo, {
          location: {
            lat: cityLookupResponse.location.latitude,
            lon: cityLookupResponse.location.longitude,
          },
      ...
      if (
        cityLookupResponse?.subdivisions &&
        cityLookupResponse?.subdivisions.length > 0
      ) {
    ...
    if (asnLookupResponse) {
      ...
      if (asnLookupResponse.autonomous_system_organization) {
        resultAs.organization = {
          name: asnLookupResponse.autonomous_system_organization,
        };
      }
    }
