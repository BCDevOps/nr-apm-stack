...
import {MaxmindAsnLookup, MaxmindCityLookup} from './maxmindLookup.service';
...
beforeEach(() => {
  ...
  myContainer.rebind<MaxmindCityLookup>(TYPES.MaxmindCityLookupService).toConstantValue({
    lookup: ():CityResponse => {
      return geoIpCityLookupVictoria;
    },
  });
  myContainer.rebind<MaxmindAsnLookup>(TYPES.MaxmindAsnLookupService).toConstantValue({
    lookup: ():AsnResponse => {
...
test('geoip - geo - full', () => {
  const geoIp = myContainer.get<GeoIpParser>(TYPES.GeoIpParser);
...
test('geoip - missing postal code', () => {
  ...
  const geoIp = myContainer.get<GeoIpService>(TYPES.GeoIpService);
  const response = geoIp.lookup('127.0.0.1');
  expect(response).toHaveProperty('geo.continent_name', geoIpCityLookupVictoria.continent.names.en);
  expect(response).toHaveProperty('geo.continent_code', geoIpCityLookupVictoria.continent.code);
  expect(response).toHaveProperty('geo.country_name', geoIpCityLookupVictoria.country.names.en);
