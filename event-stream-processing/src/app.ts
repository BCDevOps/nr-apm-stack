...
export const kinesisStreamDummyHandler = async (event: KinesisStreamEvent, context: Context): Promise<void> => {
  return myContainer.get<KinesisStreamWrapperService>(TYPES.KinesisStreamWrapperService).handle(event, context)
    .catch((error) => {
      console.error(error);
    })
    .then(() => {
