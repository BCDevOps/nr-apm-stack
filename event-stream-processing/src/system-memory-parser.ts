...
export class SystemMemoryParser implements Parser  {
    ...
    apply(record: any): void {
        ...
        if (record.system?.memory?.swap?.used?.bytes && record.system?.memory?.swap?.total && !record.system?.memory?.swap?.used?.pct) {
            lodash.set(record, 'system.memory.swap.used.pct', record.system?.memory?.swap?.used?.bytes / record.system?.memory?.swap?.total)
        }
    }
}
