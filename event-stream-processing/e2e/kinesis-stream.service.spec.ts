...
import {create as buildContainer, TYPES} from '../src/inversify.config';
import {KinesisStreamHandler} from './kinesisStreamHandler.isvc';
import {OpenSearchService, OpenSearchBulkResult} from './util/open-search.service';
import lodash from 'lodash';
import {
  AwsHttpClient, HttpBufferedResponse, HttpRequestOptions, HttpResponseWrapper} from './util/aws-http-client.isvc';
...
beforeEach(() => {
  ...
  myContainer.rebind<Randomizer>(TYPES.RandomizerService).toConstantValue({randomBytes: ()=>{
...
test('handler - success', async () => {
  ...
  // myContainer.rebind()
  myContainer.rebind<OpenSearchService>(TYPES.OpenSearchService).toConstantValue(openSearchMock);
  expect(event).toMatchSnapshot('895ca8a6-0788-41ba-b921-9edbbd3212dc');
  // eslint-disable-next-line max-len
  await expect(myContainer.get<KinesisStreamHandler>(TYPES.KinesisStreamService).handle(event, ctx)).resolves.not.toThrow();
});
...
test('handler - all errors', async () => {
  ...
  const result = await myContainer.get<KinesisStreamHandler>(TYPES.KinesisStreamService).handle(event, ctx);
...
for (let index = 1; index <= 2; index++) {
  test(`transformToElasticCommonSchema1.${index}`, async () => {
    ...
    // eslint-disable-next-line max-len
    const events = await myContainer.get<KinesisStreamHandler>(TYPES.KinesisStreamService).transformToElasticCommonSchema(event);
...
test('e2e - Apache combined format', async () => {
  ...
  // eslint-disable-next-line max-len
  const documents = await myContainer.get<KinesisStreamHandler>(TYPES.KinesisStreamService).transformToElasticCommonSchema(event);
